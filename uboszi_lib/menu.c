...
void menu_draw_all(void)
{
  ...
  GUI_SetColor(MENU_BG_COL);
  GUI_FillRect(0, 0, LCD_MAXX, FONT_H+1);
  //LCD_SetColor(MENU_BG_COL);
  //LCD_FillRect(0, 0, LCD_MAXX, FONT_H+1);
  // TOP-Menus
  ...
  if(Menu.send.data==0)
  {
    //p_gui_draw_TOP(MM_SETTING, UM_03, Menu.akt_setting); //WW: Active Menu item is assigned with red button
  }
  else
  {
    UB_Font_DrawString(MM_ITEM[MM_SETTING].xp,LINE(1),"please wait",GUI_FONT_10_ASCII,MENU_VG_COL,MENU_AK_COL);
  }
  ...
  //LCD_SetColor(MENU_BG_COL);
  //LCD_SetBkColor(MENU_BG_COL);
  //LCD_FillRect(0,LCD_MAXY-(FONT_H+1),LCD_MAXX,FONT_H+1);
  GUI_SetColor(MENU_BG_COL);
  GUI_SetBkColor(MENU_BG_COL);
  GUI_FillRect(0,LCD_MAXY-(FONT_H+1),LCD_MAXX,FONT_H+1);
...
MENU_Status_t p_gui_inc_menu(void)
{
  ...
  if(GUI.akt_menu==MM_CH1) {
    ...
    if(Menu.trigger.source==CHANNEL_SEL_CH1) { // CH1
      if(GUI.old_button!=GUI.akt_button) {
    ...
    else if(Menu.trigger.source==CHANNEL_SEL_CH2) { // CH2
      if(GUI.old_button!=GUI.akt_button) {
    ...
    if(Menu.trigger.mode==TRIGGER_MODE_SINGLE) { // "single"
      if(Menu.trigger.state==TRIGGER_STATE_READY) Menu.trigger.state=TRIGGER_STATE_WAIT2;  // von "Ready" auf "Stop"
    }
...
MENU_Status_t p_gui_dec_menu(void)
{
  ...
  if(GUI.akt_menu==MM_CH1) {
    ...
    if(Menu.trigger.edge>TRIGGER_EDGE_RISING) Menu.trigger.edge--;
  }
  else if(GUI.akt_menu==MM_TRG_MODE) {
    if(Menu.trigger.mode>TRIGGER_MODE_NORMAL) Menu.trigger.mode--;
    ret_wert=MENU_CHANGE_MODE;
  }
  else if(GUI.akt_menu==MM_TRG_VAL) {
    if(Menu.trigger.source==CHANNEL_SEL_CH1) { // CH1
    ...
    if(Menu.trigger.mode==TRIGGER_MODE_SINGLE) { // "single"
      if(Menu.trigger.state==TRIGGER_STATE_READY) Menu.trigger.state=5;  // von "Ready" auf "Stop"
    }
    else { // "normal" oder "auto"
      if(Menu.trigger.state==TRIGGER_STATE_RUN) {
        Menu.trigger.state=TRIGGER_STATE_STOP; // von "Run" auf "Stop"
...
//--------------------------------------------------------------
// pr�ft und aktuallisiert die GUI
...
void p_gui_draw_TOP(uint32_t mm_nr, const SM_Item_t um[], uint32_t um_nr)
{
  ...
  if(GUI.akt_menu==mm_nr) {
    UB_Font_DrawString(MM_ITEM[mm_nr].xp,LINE(1),buf,GUI_FONT_10_ASCII,MENU_VG_COL,MENU_AK_COL);
  }
  else {
    UB_Font_DrawString(MM_ITEM[mm_nr].xp,LINE(1),buf,GUI_FONT_10_ASCII,MENU_VG_COL,MENU_BG_COL);
  }
}
...
void p_menu_draw_BOT(uint32_t mm_nr, const SM_Item_t um[], uint32_t um_nr, uint32_t mode)
{
  ...
  if(mode==0) {
    ...
    if(Menu.trigger.source==CHANNEL_SEL_CH2) {
  ...
  if(GUI.akt_menu==mm_nr) {
    UB_Font_DrawString(MM_ITEM[mm_nr].xp,LINE(27),buf,GUI_FONT_10_ASCII,MENU_VG_COL,MENU_AK_COL);
  }
  else {
    UB_Font_DrawString(MM_ITEM[mm_nr].xp,LINE(27),buf,GUI_FONT_10_ASCII,MENU_VG_COL,MENU_BG_COL);
  }
}
...
void p_menu_draw_BOT_TRG(void)
{
  ...
  p_menu_draw_BOT(MM_TRG_RESET,UM_10,Menu.trigger.state,7);
}
...
void p_menu_draw_BOT_CUR(void)
{
  ...
  if((Menu.cursor.mode==1) || (Menu.cursor.mode==2)) {
    ...
    sprintf(buf,"~=%sV",bval);
    UB_Font_DrawString(33*FONT_W,LINE(27),buf,GUI_FONT_10_ASCII,MENU_VG_COL,MENU_BG_COL);
  }
...
void p_menu_draw_BOT_VERSION(void)
{
  UB_Font_DrawString(10,LINE(27),"STM32F746G-Oszi  |  WW  |  V:0.8  |  20.03.2019",GUI_FONT_10_ASCII,MENU_VG_COL,MENU_BG_COL);
}
...
void p_menu_draw_BOT_HELP(void)
{
  UB_Font_DrawString(10,LINE(27),"CH1=PA0=ARD_A0  |  CH2=PF10=ARD_A1  |  TX=PA9=VCP",GUI_FONT_10_ASCII,MENU_VG_COL,MENU_BG_COL);
}


//--------------------------------------------------------------
// Value�nderung (incrementieren ohne Vorzeichen)
...
//--------------------------------------------------------------
// Value�nderung (decrementieren ohne Vorzeichen)
// Wertebereich : 0 bis 4095
// erhoeht automatisch die schrittweite
//--------------------------------------------------------------
uint16_t dec_uintval(uint16_t wert, uint16_t startwert)
...
//--------------------------------------------------------------
// Value�nderung (incrementieren mit Vorzeichen)
...
//--------------------------------------------------------------
// Value�nderung (decrementieren mit Vorzeichen)
// Wertebereich : -200 bis +200
// erhoeht automatisch die schrittweite
//--------------------------------------------------------------
int16_t dec_intval(int16_t wert, uint16_t startwert)
