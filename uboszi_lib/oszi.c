...
void oszi_init(void)
{
  ...
  p_oszi_send_uart("OSZI 7 STM32F746 [WW+UB]");
  if(check==1)
  {
    // Touch init error
    UB_LCD_FillLayer(BACKGROUND_COL);
    UB_Font_DrawString(10,10,"Touch ERR",GUI_FONT_10_ASCII,FONT_COL,BACKGROUND_COL);
    while(1);
  }
  else if(check==2)
  {
    ...
    UB_LCD_FillLayer(BACKGROUND_COL);
    UB_Font_DrawString(10,10,"Wrong ADC Array-LEN",GUI_FONT_10_ASCII,FONT_COL,BACKGROUND_COL);
    while(1);
  }
...
void oszi_process(void)
{
  ...
  if(UB_Button_OnClick(BTN_USER)==true)
  {
	...
	if(Menu.trigger.mode==TRIGGER_MODE_SINGLE)
	{ // "single"
	  if(Menu.trigger.state==TRIGGER_STATE_READY)
	  {
		Menu.trigger.state=TRIGGER_STATE_WAIT2;  // von "Ready" auf "Stop"
  ...
  if(status!=MENU_NO_CHANGE)
  {
	...
	if(status==MENU_CHANGE_MODE)
	{
	  ...
	  if(Menu.trigger.mode!=TRIGGER_MODE_SINGLE)
	  {
		Menu.trigger.state=TRIGGER_STATE_RUN;
	  }
	  else
	  {
		Menu.trigger.state=TRIGGER_STATE_WAIT;
	  }
  ...
  if(Menu.trigger.mode==TRIGGER_MODE_AUTO)
  {
	//--------------------------------------
	// Trigger-Mode = "AUTO"
	...
	if(Menu.trigger.state==TRIGGER_STATE_RUN)
	{
	  if (wwTriggerQuater==1 || wwTriggerQuater==3 ) //LL_DMA_GetCurrentTargetMem(ADC1_DMA, ADC1_DMA_STREAM) == LL_DMA_CURRENTTARGETMEM0)
	  {
...
//          if(Menu.fft.mode!=0) fft_calc();
  ...
  else if(Menu.trigger.mode==TRIGGER_MODE_NORMAL)
  {
	//--------------------------------------
	// Trigger-Mode = "NORMAL"
...
//            if(Menu.fft.mode!=0) fft_calc();
  ...
  else
  {
	...
	//--------------------------------------
	if(Menu.trigger.state==TRIGGER_STATE_WAIT)
	{
	  // warten auf Trigger-Event
	  if(ADC_UB.status==ADC_READY)
	  {
		Menu.trigger.state=TRIGGER_STATE_READY;
		//UB_Led_Toggle(LED_RED);
		p_oszi_sort_adc();
//            p_oszi_fill_fft();
//            if(Menu.fft.mode!=0) fft_calc();
	...
	else if(Menu.trigger.state==TRIGGER_STATE_WAIT2)
	{
...
void p_oszi_sw_init(void)
{
  ...
  Menu.trigger.source=CHANNEL_SEL_NONE;  // trigger=No Channel
  Menu.trigger.edge=TRIGGER_EDGE_RISING;    // hi-flanke
  Menu.trigger.mode=TRIGGER_MODE_AUTO;    // auto
  Menu.trigger.state=TRIGGER_STATE_RUN;
...
void p_oszi_draw_scale(void)
{
  ...
  for(m=0;m<=SCALE_H;m+=SCALE_SPACE)
  {
    ...
    {
      UB_Graphic2D_DrawPixelNormal(n+xs,m+ys,SCALE_COL);
    }
  }

  //---------------------------------------------
  // Y-Achse (Horizontale Mittel-Linie)
  //---------------------------------------------
  signed_int=SCALE_Y_MITTE+ys;
  p_oszi_draw_line_h(LCD_MAXY-signed_int,SCALE_COL,0);

  //---------------------------------------------
  // X-Achse (Vertikale Mittel-Linie)
  //---------------------------------------------
  signed_int=SCALE_X_MITTE+xs;
  p_oszi_draw_line_v(signed_int,SCALE_COL,0);
 
  //---------------------------------------------
  // Umrandung
  //---------------------------------------------
  LCD_SetColor(SCALE_COL);
  // unterste linie
  //UB_Graphic2D_DrawStraightDMA(xs-1,ys-1,SCALE_W+2,LCD_DIR_HORIZONTAL,SCALE_COL);
  GUI_DrawHLine(xs-1, LCD_MAXY-(ys-1), SCALE_W+2);
  // oberste linie
  //UB_Graphic2D_DrawStraightDMA(xs+SCALE_H+1,ys-1,SCALE_W+2,LCD_DIR_HORIZONTAL,SCALE_COL);
  GUI_DrawHLine(xs-1, LCD_MAXY-(ys+SCALE_H+1), SCALE_W+2);
  // linke linie
  //UB_Graphic2D_DrawStraightDMA(xs-1,ys-1,SCALE_H+2,LCD_DIR_VERTICAL,SCALE_COL);
  GUI_DrawVLine(xs-1, ys-1, SCALE_H+2);
  // rechte linie
  //UB_Graphic2D_DrawStraightDMA(xs-1,ys+SCALE_W+1,SCALE_H+2,LCD_DIR_VERTICAL,SCALE_COL);
  GUI_DrawVLine(xs+SCALE_W+2, ys-1, SCALE_H+2);
  ...
  if(Menu.trigger.source==CHANNEL_SEL_CH1)
  {
    ...
    signed_int+=SCALE_Y_MITTE+SCALE_START_Y+Menu.ch1.position;
    if(signed_int<SCALE_START_Y) signed_int=SCALE_START_Y;
    if(signed_int>SCALE_MY_PIXEL) signed_int=SCALE_MY_PIXEL;

    p_oszi_draw_line_h(LCD_MAXY-signed_int,ADC_CH1_COL,1);
    UB_Font_DrawString(0,LCD_MAXY-(signed_int+3),"T",GUI_FONT_8_ASCII,ADC_CH1_COL,BACKGROUND_COL);
  }
  else if(Menu.trigger.source==CHANNEL_SEL_CH2)
  {
    signed_int=oszi_adc2pixel(Menu.trigger.value_ch2, Menu.ch2.faktor);
    signed_int+=SCALE_Y_MITTE+SCALE_START_Y+Menu.ch2.position;
    if(signed_int<SCALE_START_Y) signed_int=SCALE_START_Y;
    if(signed_int>SCALE_MY_PIXEL) signed_int=SCALE_MY_PIXEL;

    p_oszi_draw_line_h(LCD_MAXY-signed_int,ADC_CH2_COL,1);
    UB_Font_DrawString(0,LCD_MAXY-(signed_int+3),"T",GUI_FONT_8_ASCII,ADC_CH2_COL,BACKGROUND_COL);
  }
  ...
  if(Menu.cursor.mode==1)
  {
    ...
    p_oszi_draw_line_h(LCD_MAXY-signed_int,CURSOR_COL,2);
    UB_Font_DrawString(LCD_MAXX-8,LCD_MAXY-(signed_int-3),"A",GUI_FONT_8_ASCII,CURSOR_COL,BACKGROUND_COL);
    ...
    p_oszi_draw_line_h(LCD_MAXY-signed_int,CURSOR_COL,2);
    UB_Font_DrawString(LCD_MAXX-8,LCD_MAXY-(signed_int-3),"B",GUI_FONT_8_ASCII,CURSOR_COL,BACKGROUND_COL);
  }
  ...
  else if(Menu.cursor.mode==3)
  {
    ...
    UB_Font_DrawString(signed_int-3,15,"A",GUI_FONT_8_ASCII,CURSOR_COL,BACKGROUND_COL);
    ...
    p_oszi_draw_line_v(signed_int,CURSOR_COL,2);
    UB_Font_DrawString(signed_int-3,15,"B",GUI_FONT_8_ASCII,CURSOR_COL,BACKGROUND_COL);
  }
  else if(Menu.cursor.mode==4)
  {
    ...
    signed_int+=FFT_START_X+1;
    if(signed_int<FFT_START_X) signed_int=FFT_START_X;
    if(signed_int>(FFT_START_X+FFT_VISIBLE_LENGTH)) signed_int=(FFT_START_X+FFT_VISIBLE_LENGTH);
...
//--------------------------------------------------------------
void p_oszi_draw_line_h(uint16_t yp, uint16_t c, uint16_t m)
{
  uint32_t n,t;
...
// an "yp", mit Farbe "c" und Mode "m"
//--------------------------------------------------------------
void p_oszi_draw_line_v(uint16_t xp, uint16_t c, uint16_t m)
{
  uint32_t n,t;
...
//--------------------------------------------------------------
// sortiert die Daten der ADC-Kan�le von Buffer_A und Buffer_B
// in den Buffer_C um
// die Daten werden so sortiert, das das Trigger-Event in der Mitte
// vom Datenbreich liegt (das ist sp�ter die Mitte vom Screen)
//
// Der Trigger-Punkt kann in einem der 4 Quadranten
// vom Daten-Puffer liegen
// Quadrant-1 = erste h�lfte vom Buffer-A
// Quadrant-2 = zweite h�lfte vom Buffer-A
// Quadrant-3 = erste h�lfte vom Buffer-B
// Quadrant-4 = zweite h�lfte vom Buffer-B
//--------------------------------------------------------------
void p_oszi_sort_adc(void)
{
  uint32_t n=0;
  ...
  if(ADC_UB.trigger_quarter==1)
  {
    ...
    for(n=0;n<anz1;n++)
    {
      ...
      wert=*(((uint16_t*)ADC_DMA_Buffer_B_1)+((start+n)*ADC1d_ANZ));
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+((start+n)*ADC1d_ANZ)) = wert;
      wert=*(((uint16_t*)ADC_DMA_Buffer_B_1)+((start+n)*ADC1d_ANZ+1));
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+((start+n)*ADC1d_ANZ+1)) = wert;
      wert=*(((uint16_t*)ADC_DMA_Buffer_B_1)+((start+n)*ADC1d_ANZ+2));
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+((start+n)*ADC1d_ANZ+2)) = wert;
#else
      wert=ADC_DMA_Buffer_B[(start+n)*ADC1d_ANZ];
      ADC_DMA_Buffer_C[n*ADC1d_ANZ]=wert;
      wert=ADC_DMA_Buffer_B[(start+n)*ADC1d_ANZ+1];
      ADC_DMA_Buffer_C[n*ADC1d_ANZ+1]=wert;
      wert=ADC_DMA_Buffer_B[((start+n)*ADC1d_ANZ)+2];
      ADC_DMA_Buffer_C[(n*ADC1d_ANZ)+2]=wert;
#endif
    }
    //-------------------------------
...
#ifdef WW_USE_AUDIO_BUFFER_FOR_ADC
      wert=*(((uint16_t*)ADC_DMA_Buffer_B_1)+((start+n)*ADC1d_ANZ));
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+((n+anz1)*ADC1d_ANZ)) = wert;
      wert=*(((uint16_t*)ADC_DMA_Buffer_B_1)+((start+n)*ADC1d_ANZ+1));
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+((start+n)*ADC1d_ANZ+1)) = wert;
      wert=*(((uint16_t*)ADC_DMA_Buffer_B_1)+((start+n)*ADC1d_ANZ+2));
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+(((start+n)*ADC1d_ANZ)+2)) = wert;
#else
      wert=ADC_DMA_Buffer_A[(start+n)*ADC1d_ANZ];
      ADC_DMA_Buffer_C[(n+anz1)*ADC1d_ANZ]=wert;
      wert=ADC_DMA_Buffer_A[(start+n)*ADC1d_ANZ+1];
      ADC_DMA_Buffer_C[(n+anz1)*ADC1d_ANZ+1]=wert;
      wert=ADC_DMA_Buffer_A[((start+n)*ADC1d_ANZ)+2];
      ADC_DMA_Buffer_C[((n+anz1)*ADC1d_ANZ)+2]=wert;
#endif
    }
  }
  else if(ADC_UB.trigger_quarter==2)
  {
    ...
    {
#ifdef WW_USE_AUDIO_BUFFER_FOR_ADC
      wert=*(((uint16_t*)ADC_DMA_Buffer_A_1)+((start+n)*ADC1d_ANZ));
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+(n*ADC1d_ANZ)) = wert;
      wert=*(((uint16_t*)ADC_DMA_Buffer_A_1)+((start+n)*ADC1d_ANZ+1));
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+(n*ADC1d_ANZ+1)) = wert;
      wert=*(((uint16_t*)ADC_DMA_Buffer_A_1)+((start+n)*ADC1d_ANZ+2));
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+((n*ADC1d_ANZ)+2)) = wert;
#else
      wert=ADC_DMA_Buffer_A[(start+n)*ADC1d_ANZ];
      ADC_DMA_Buffer_C[n*ADC1d_ANZ]=wert;
      wert=ADC_DMA_Buffer_A[(start+n)*ADC1d_ANZ+1];
    ...
    for(n=0;n<anz2;n++)
    {
      ...
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+((n+anz1)*ADC1d_ANZ)) = wert;
      wert=*(((uint16_t*)ADC_DMA_Buffer_B_1)+((start+n)*ADC1d_ANZ+1));
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+((n+anz1)*ADC1d_ANZ+1)) = wert;
      wert=*(((uint16_t*)ADC_DMA_Buffer_B_1)+((start+n)*ADC1d_ANZ+2));
      *(((uint16_t*)ADC_DMA_Buffer_C_1)+((n+anz1)*ADC1d_ANZ+2)) = wert;
#else
      wert=ADC_DMA_Buffer_B[(start+n)*ADC1d_ANZ];
      ADC_DMA_Buffer_C[(n+anz1)*ADC1d_ANZ]=wert;
      wert=ADC_DMA_Buffer_B[(start+n)*ADC1d_ANZ+1];
...
void p_oszi_fill_fft(void)
{
  ...
  if(Menu.fft.mode==1) {
    ...
    for(n=0;n<FFT_LENGTH;n++) {
      if(m<SCALE_W) {
        FFT_DATA_IN[n]=(float32_t)((ADC_DMA_Buffer_C[(m*ADC1d_ANZ)+1]-2048.0)/1000.0);
      }
      else {
        FFT_DATA_IN[n]=0.0;
      }
    ...
    for(n=0;n<FFT_LENGTH;n++) {
      if(m<SCALE_W) {
        FFT_DATA_IN[n]=(float32_t)((ADC_DMA_Buffer_C[(m*ADC1d_ANZ)+2]-2048.0)/1000.0);
      }
      else {
        FFT_DATA_IN[n]=0.0;
      }
...
//--------------------------------------------------------------
// zeichnet die Daten der zwei ADC-Kan�le (und die FFT)
//--------------------------------------------------------------
void p_oszi_draw_adc(void)
{
  uint32_t n=0;
  ...
  // startwerte
#ifdef WW_USE_AUDIO_BUFFER_FOR_ADC
  ch1_wert1=oszi_adc2pixel(*(((uint16_t*)ADC_DMA_Buffer_C_1)+1), Menu.ch1.faktor);
#else
  ch1_wert1=oszi_adc2pixel(ADC_DMA_Buffer_C[0+1], Menu.ch1.faktor);
#endif
  ch1_wert1+=SCALE_Y_MITTE+SCALE_START_Y+Menu.ch1.position;
  if(ch1_wert1<SCALE_START_Y) ch1_wert1=SCALE_START_Y;
  if(ch1_wert1>SCALE_MY_PIXEL) ch1_wert1=SCALE_MY_PIXEL;

#ifdef WW_USE_AUDIO_BUFFER_FOR_ADC
  ch2_wert1=oszi_adc2pixel(*(((uint16_t*)ADC_DMA_Buffer_C_1)+2), Menu.ch2.faktor);
#else
  ch2_wert1=oszi_adc2pixel(ADC_DMA_Buffer_C[1+1], Menu.ch2.faktor);
#endif
  ch2_wert1+=SCALE_Y_MITTE+SCALE_START_Y+Menu.ch2.position;
  if(ch2_wert1<SCALE_START_Y) ch2_wert1=SCALE_START_Y;
  if(ch2_wert1>SCALE_MY_PIXEL) ch2_wert1=SCALE_MY_PIXEL;
  ...
  for(n=1;n<SCALE_W;n++)
  {
    ...
    {
#ifdef WW_USE_AUDIO_BUFFER_FOR_ADC
      ch1_wert2=oszi_adc2pixel(*(((uint16_t*)ADC_DMA_Buffer_C_1)+(n*ADC1d_ANZ+1)), Menu.ch1.faktor);
#else
      ch1_wert2=oszi_adc2pixel(ADC_DMA_Buffer_C[n*ADC1d_ANZ+1], Menu.ch1.faktor);
#endif
      ch1_wert2+=SCALE_Y_MITTE+SCALE_START_Y+Menu.ch1.position;
      if(ch1_wert2<SCALE_START_Y) ch1_wert2=SCALE_START_Y;
      if(ch1_wert2>SCALE_MY_PIXEL) ch1_wert2=SCALE_MY_PIXEL;
      UB_Graphic2D_DrawLineNormal(SCALE_START_X+n,LCD_MAXY-ch1_wert1,SCALE_START_X+n+1,LCD_MAXY-ch1_wert2,ADC_CH1_COL);
      ch1_wert1=ch1_wert2;
    }
    ...
    {
#ifdef WW_USE_AUDIO_BUFFER_FOR_ADC
      ch2_wert2=oszi_adc2pixel(*(((uint16_t*)ADC_DMA_Buffer_C_1)+(n*ADC1d_ANZ+2)), Menu.ch2.faktor);
#else
      ch2_wert2=oszi_adc2pixel(ADC_DMA_Buffer_C[(n*ADC1d_ANZ)+2], Menu.ch2.faktor);
#endif
      ch2_wert2+=SCALE_Y_MITTE+SCALE_START_Y+Menu.ch2.position;
      if(ch2_wert2<SCALE_START_Y) ch2_wert2=SCALE_START_Y;
      if(ch2_wert2>SCALE_MY_PIXEL) ch2_wert2=SCALE_MY_PIXEL;
      UB_Graphic2D_DrawLineNormal(SCALE_START_X+n,LCD_MAXY-ch2_wert1,SCALE_START_X+n+1,LCD_MAXY-ch2_wert2,ADC_CH2_COL);
      ch2_wert1=ch2_wert2;
    }
  }
...
void p_oszi_send_data(void)
{
  ...
  if((Menu.send.mode==0) || (Menu.send.mode==1)) {
    ...
    for(n=0;n<SCALE_W;n++) {
#ifdef WW_USE_AUDIO_BUFFER_FOR_ADC
      wert1=*(((uint16_t*)ADC_DMA_Buffer_C_1)+(n*ADC1d_ANZ+1));
#else
      wert1=ADC_DMA_Buffer_C[n*ADC1d_ANZ+1];
#endif
      sprintf(buf,"%d",wert1);
      p_oszi_send_uart(buf);
    }
    ...
    for(n=0;n<SCALE_W;n++) {
#ifdef WW_USE_AUDIO_BUFFER_FOR_ADC
      wert2=*(((uint16_t*)ADC_DMA_Buffer_C_1)+(n*ADC1d_ANZ+2));
#else
      wert2=ADC_DMA_Buffer_C[(n*ADC1d_ANZ)+2];
#endif
      sprintf(buf,"%d",wert2);
      p_oszi_send_uart(buf);
    }
...
void p_oszi_send_uart(char *ptr)
{
  UB_Uart_SendString(COM1x,ptr,CRLF);
}
...
void p_oszi_send_screen(void)
{
  ...
  for(n=0;n<BMP_HEADER_LEN;n++) {
    UB_Uart_SendByte(COM1x,BMP_HEADER[n]);
  }
...
void setTimeSelection(int btnIndex)
{
   Menu.timebase.value = btnIndex-1;
   ADC_change_Frq(Menu.timebase.value);
}

int  getTimeSelection(void)
{
   return Menu.timebase.value;
}

int getSelextedChannel(void)
{
	return 0;
}

int getChannelVoltIndex(int channelIndex)
{
	return Menu.ch1.faktor;
}

int isChannelEnabled(int channelIndex)
{
	return !(Menu.ch1.visible);
}

void setChannelConfig(CHANNEL_SELECTION_e ch, int voltIndex, int enabled)
{
	// Adaption to the original UB implementation
	voltIndex -= 1;
	enabled = !enabled;

	if( ch==CHANNEL_SEL_CH1 )
	{
	   Menu.ch1.faktor = voltIndex;
	   Menu.ch1.visible = enabled;
	}
	else if( ch==CHANNEL_SEL_CH2 )
	{
	   Menu.ch2.faktor = voltIndex;
	   Menu.ch2.visible = enabled;
	}
}

void osziHandleArrowKey(ARROW_KEY_t key)
{
	int ami = getActiveMenuItem();
	switch(ami)
	{
	case 2: // CH1
		switch(key)
...
{
   Menu.trigger.source = ch;
   if( ch == CHANNEL_SEL_NONE )
      Menu.trigger.mode = TRIGGER_MODE_AUTO;
}

void osziSetTriggerEdge(TRIGGER_EDGE_e te)
{
   Menu.trigger.edge = te;
}

void osziSetTriggerMode(TRIGGER_MODE_e tm)
{
   Menu.trigger.mode = tm;
}
...
{
	switch((TIME_SETTING_e)(Menu.timebase.value+1))
	{
	case TIME_SET_5s: return 5;
	case TIME_SET_2s: return 12;
	case TIME_SET_1s: return 25;
	case TIME_SET_05s: return 50;
	case TIME_SET_02s: return 125;
	case TIME_SET_01s: return 250;
	case TIME_SET_50ms: return 500;
	case TIME_SET_20ms: return 1250;
	case TIME_SET_10ms: return 2500;
	case TIME_SET_5ms: return 5000;
	case TIME_SET_2ms: return 12500;
	case TIME_SET_1ms: return 25000;
	case TIME_SET_500us: return 50000;
	case TIME_SET_200us: return 125000;
	case TIME_SET_100us: return 250000;
	case TIME_SET_50us: return 500000;
	case TIME_SET_25us: return 1000000;
	}
}
...
/**
  ...
  */
uint8_t  OSZI_SetRecordCallback( OsciBufferTransfer_CallBack cb)
{
  /* Hook into the DMA interrupt handling */
  ADC_UB.recordCallback = cb;
  return 0;
}
...
{
  /* Unhook the DMA interrupt handling */
  ADC_UB.recordCallback = 0;
  //TODO:WW: Get current running DMA semi transfer and trigger final recording event
  return 0;
}
